...
export let assets: BuildRouteHandler<"GET", typeof routes.assets> = async ({
  ...
  return serveFile(path.join(assetsDir, params.path));
};

export let static_: BuildRouteHandler<"GET", typeof routes.images> = async ({
  params,
}) => {
  return serveFile(path.join(publicDir, params.path));
};
